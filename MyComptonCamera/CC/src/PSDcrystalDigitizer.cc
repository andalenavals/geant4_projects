...
PSDcrystalDigitizer::PSDcrystalDigitizer(G4String aName) :
  G4VDigitizerModule(aName) ,
  //These two are names for digits and hits collections
  hitsCollName("psdcrystalHCollection") ,
  digiCollectionName("psdcrystalDCollection") ,
  // ************************************************* //
...
void PSDcrystalDigitizer::Digitize()
{
  ...
  const PSDcrystalHitsCollection* hitCollection = static_cast<const PSDcrystalHitsCollection*>(digMan->GetHitsCollection(HitCollID));
  
  PSDcrystalDigitsCollection* digiCollection = 0;
  ...
  if ( hitCollection )
    {
      ...
      digiCollection = new PSDcrystalDigitsCollection("PSDcrystalDigitizer",digiCollectionName);
      
      //Initialize with NULL pointers
      std::vector<PSDcrystalDigit*> digitsMap(hitCol_size);
      /***************************************************************/
      ...
      for (G4int i=0 ; i<hitCol_size; i++ )
	{
	  ...
	  //For each Hit we now get to which strip it belongs
          ...
          PSDcrystalHit* aHit = (*hitCollection)[i];
	  //	  aHit->Print();

	  PSDcrystalDigit* newDigi = new PSDcrystalDigit(i);       //Create empty digits
	  digitsMap[i] = newDigi;     
    ...
    {
      G4cerr<<" PSDcrystalDigitizer::Digitize: Could not found Hit collection with name:"<<hitsCollName<<G4endl;
    }
  ...
  for ( size_t d = 0 ; d < digiCollection->GetSize(); ++d )
    {
      ...
      PSDcrystalDigit* digi = static_cast< PSDcrystalDigit* >( digiCollection->GetDigi(d) );
...
void PSDcrystalDigitizer::MakeCrosstalk(std::vector< std::vector< PSDcrystalDigit*> >& digitsMap )
{
  //We need to work heavily on this routine 
  G4cout<<"PSDcrystalDigitizer::MakeCrosstalk"<<G4endl;
...
void PSDcrystalDigitizer::ResetValues(void)
{
  SetDetectorID(-1);
  SetCharge(0.0);
  SetLocalTime(0.0);
