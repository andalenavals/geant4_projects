...
void MyAnalysisManager::DrawAnimation(int i, int pasos )
{
  paso+=1;
  if(paso%pasos==0)
    {
...
void MyAnalysisManager::SetFrame(double x0, double y0,  double x1, double y1, const char * title , const char * titlex ,const char * titley)
{
  frame = gPad->DrawFrame(x0, y0, x1, y1 );
  DefinirTitulos(frame, title, titlex,titley);
}
