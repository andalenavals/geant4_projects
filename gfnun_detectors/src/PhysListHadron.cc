...
void PhysListHadron::ConstructProcess()
{
  ...
  if (fThermal) theElasticNeutron->SetMinEnergy(4*eV);
  ...
  if (fThermal) fTheNeutronElasticProcess->AddDataSet(dataSet1b);
  G4NeutronHPThermalScattering* model1b = new G4NeutronHPThermalScattering();
  if (fThermal) fTheNeutronElasticProcess->RegisterMe(model1b);
  // inelastic 
